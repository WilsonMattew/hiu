...
class Payment_model extends CI_Model {
    ...
    public function paypal_payment($paymentID = "", $paymentToken = "", $payerID = "") {
      $paypal_keys = json_decode(get_settings('paypal_keys'));
      $paypalEnv   = $paypal_keys[0]->mode; // Or 'production'

      if ($paypal_keys[0]->mode == 'sandbox') {
          $paypalURL       = 'https://api.sandbox.paypal.com/v1/';
          $paypalClientID = $paypal_keys[0]->sandbox_client_id;
          $paypalSecret   = $paypal_keys[0]->sandbox_secret_key;
      } else {
          $paypalURL       = 'https://api.paypal.com/v1/';
          $paypalClientID = $paypal_keys[0]->production_client_id;
          $paypalSecret   = $paypal_keys[0]->production_secret_key;
      }
      ...
      if($result && $result->state == 'approved'){
        $return_response['payment_status'] = true;
        $return_response['paid_amount'] = $result->transactions[0]->amount->total;
      }else{
        $return_response = array('payment_status'=>false, 'paid_amount'=>0);
      }
                ...
                if(empty($api_error) && $intent){
                    ...
                    if($intent->status == 'succeeded'){
                        ...
                        if($payment_status == 'succeeded'){
                            $status_msg = get_phrase("payment_has_been_Successful");
                        }else{
                            $status_msg = get_phrase("payment_failed");
                        }
                    }else{
                        $status_msg = get_phrase("transaction_failed");;
                    }
                }else{
  ...
  public function razorpayPrepareData($order_id = "", $user_id = "")
  {
    $order_details = $this->crud_model->get_orders($order_id)->row_array();
    $payable_amount = $order_details['total_price'] + get_store_settings('delivery_charge');
    $user_details = $this->crud_model->get_users($order_details['user_id'])->row_array();
    ...
    $data = array(
      ...
      "image" => system_image('uploads/system_images/'.get_store_settings('small_logo')),
      "prefill" => array(
