...
if($package['package_type'] == 'days'){
    $product_name = get_phrase('package_of_'.$package['days'].'_days_on_'.get_settings('system_title'));
}else{
    $product_name = get_phrase($package['package_type'].'_on_'.get_settings('system_title'));
}
...
$stripeAmount = round($package['price']*100, 2);
...
define('STRIPE_SUCCESS_URL', site_url('membership/payment_success/stripe/'.$package['package_id'].'/'.$package['price']));
define('STRIPE_CANCEL_URL', site_url('membership'));
