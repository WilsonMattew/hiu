<?php
$watched_classes = $this->frontend_model->get_watch_histories_by_user();
foreach($watched_classes->result_array() as $watched_classe):
	$class_details = $this->crud_model->get_classes($watched_classe['class_id'])->row_array();
	$category = $this->crud_model->get_categories($class_details['category_id'])->row_array(); ?>
	<div class="row mx-1" id="watched_class_<?php echo $class_details['class_id'] ?>">
		...
		<div class="col-md-9 col-lg-10 py-2">
			<div>
				<p class="class-card-title my-1 w-100">
					...
					<a href="javascript:;" onclick="confirm_modal('<?php echo site_url('classes/remove_watching_class?class_id='.$class_details['class_id']); ?>', '#watched_class_<?php echo $class_details['class_id'] ?>')" class="float-end text-decoration-none text-danger text-14 fw-500"><i class="bi bi-trash"></i><?php echo get_phrase('remove'); ?></a>
				</p>
