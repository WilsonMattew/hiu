...
<div class="card pt-4">
  <div class="card-body">
    <ul class="list-group list-group-dividered list-group-full">
      <?php foreach($following->result_array() as $key => $row): ?>
        <?php $user_row = $this->crud_model->get_users($row['user_id'])->row_array(); ?>
          ...
          <div class="row">
            <div class="col-sm-2 col-md-1 text-center">
              <a href="<?php echo site_url('user/profile/'.$user_row['user_id']); ?>">
                <img src="<?php echo get_user_image($row['user_id']); ?>" width="40px" class="rounded-circle">
              </a>
            </div>
            ...
            <div class="col-sm-5 col-md-5 text-center text-md-end">
              <a href="javascript:;" onclick="follow2(this, '.<?= $key; ?>followBtn', '<?php echo $user_row['user_id']; ?>')" class="<?= $key; ?>followingBtn btn btn-primary rounded-pill border-primary fw-500 py-1 px-4 <?php if($is_followed <= 0) echo 'd-hidden'; ?>">
                <?php echo get_phrase('following'); ?>
              </a>

              <a href="javascript:;" onclick="follow2(this, '.<?= $key; ?>followingBtn', '<?php echo $user_row['user_id']; ?>')" class="<?= $key; ?>followBtn btn btn-outline-primary rounded-pill border border-2 border-primary fw-500 py-1 px-4 <?php if($is_followed > 0) echo 'd-hidden'; ?>">
