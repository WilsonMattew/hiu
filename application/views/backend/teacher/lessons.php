<div class="row">
  <div class="col-lg-8 col-lg-offset-2">
    <h5 class="mb-15px pb-15px">
      ...
      <a class="btn btn-primary btn-sm float-right" href="javascript:;" onclick="showAjaxModal('<?php echo site_url('teacher/lesson_add/'.$class_details['class_id']); ?>', '<?php echo get_phrase('add_lesson'); ?>')">
    ...
    <ul id="sortable" class="list-group">
      <?php foreach($lessons->result_array() as $lesson): ?>
        <li class="list-group-item mb-10px" id="<?php echo $lesson['lesson_id']; ?>">
          ...
          <a href="javascript:;" class="float-right text-danger" onclick="confirm_modal('<?php echo site_url('teacher/lesson_delete/'.$class_details['class_id'].'/'.$lesson['lesson_id']); ?>')" data-toggle="tooltip" title="<?php echo get_phrase('delete'); ?>">
            <i class="entypo-trash"></i>
          </a>

          <a href="javascript:;" class="float-right" onclick="showAjaxModal('<?php echo site_url('teacher/lesson_edit/'.$class_details['class_id'].'/'.$lesson['lesson_id']); ?>', '<?php echo get_phrase('edit_lesson'); ?>')" data-toggle="tooltip" title="<?php echo get_phrase('edit'); ?>">
  ...
  $(function() {
    $('#sortable').sortable({
        ...
        update: function(event, ui) {
          ...
          $.ajax({
            type: 'post',
            url: '<?php echo site_url('teacher/sort_lesson'); ?>',
