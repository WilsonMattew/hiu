<form class="realtime-form" action="<?php echo site_url('teacher/lesson_add/added/'.$class_id); ?>" method="post" enctype="multipart/form-data">
	<div class="row mb-15px">
		<label for="lesson_title" class="col-sm-3 control-label text-right pt-5px"><?php echo get_phrase('lesson_title'); ?>*</label>
		<div class="col-sm-7">
			<input type="text" class="form-control" name="lesson_title" id="lesson_title" placeholder="<?php echo get_phrase('lesson_title'); ?>" required>
		</div>
	</div>
	...
	<div class="row mb-15px">
		...
		<div class="col-sm-7">
			<select class="form-control" name="lesson_type" id="lesson_type" onchange="showSrcField(this)" required>
				...
				<option value="html5_video_url"><?php echo get_phrase('html5_video_url'); ?></option>
				<option value="video_file"><?php echo get_phrase('video_file'); ?></option>
			</select>
		</div>
	</div>

	<div class="row mb-15px lesson_src_field d-hidden" id="for_video_url">
		<label for="video_url" class="col-sm-3 control-label text-right pt-5px"><?php echo get_phrase('video_url'); ?>*</label>
		<div class="col-sm-7">
			<input id="video_url" onblur="get_video_duration(this)" type="url" name="video_url" class="form-control" placeholder="<?php echo get_phrase('enter_your_video_url'); ?>">
		</div>
	</div>

	<div class="row mb-15px lesson_src_field d-hidden" id="for_video_file">
	...
	<div class="row mb-15px">
		...
		<div class="col-sm-7">
			<input type="checkbox" name="is_free_lesson" value="1" id="is_free_lesson">
	...
	<div class="row mb-15px">
		...
		<div class="col-sm-7">
			<button type="submit" class="btn btn-primary"><?php echo get_phrase('upload_lesson'); ?></button>
		</div>
	</div>
	...
	function get_video_duration(e){
		...
		if(lesson_type == 'youtube' || lesson_type == 'vimeo'){
			...
			if(validURL(video_url)){
				$.ajax({
					type: 'post',
					url: '<?php echo site_url('teacher/get_video_duration_by_type/'); ?>',
